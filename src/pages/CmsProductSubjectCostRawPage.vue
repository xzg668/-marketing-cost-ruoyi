<template>
  <cms-cost-page-shell
    title="CMS 科目成本原始数据"
    description="辅助人员工资和辅料科目来源，材料计价原始单位为分。"
    trace-nav
  >
    <section class="cms-query-panel">
      <el-form class="cms-query-form" :inline="true" :model="filters" label-width="72px">
        ...
        <el-form-item label="科目编码">
          <el-input v-model="filters.subjectCode" clearable placeholder="二级科目编码" />
        </el-form-item>
        <el-form-item label="科目名称">
          <el-input v-model="filters.subjectName" clearable placeholder="辅助人员工资 / 辅助材料" />
        </el-form-item>
    ...
    <el-table class="cms-raw-table" :data="rows" stripe border v-loading="loading">
      ...
      <el-table-column prop="firstSubjectName" label="一级科目" min-width="130" show-overflow-tooltip />
      <el-table-column prop="secondSubjectCode" label="二级科目编码" width="120" />
      <el-table-column prop="secondSubjectName" label="二级科目" min-width="150" show-overflow-tooltip />
      <el-table-column prop="thirdSubjectName" label="三级科目" min-width="150" show-overflow-tooltip />
      <el-table-column prop="lastSubjectName" label="末级科目" min-width="150" show-overflow-tooltip />
      <el-table-column prop="materialPrice" label="材料计价(分)" width="120" />
      <el-table-column prop="materialPriceYuan" label="材料计价(元)" width="120" />
      <el-table-column prop="buildFlag" label="建机标识" width="100" />
      <el-table-column prop="sourceRowId" label="CMS原始ID" width="130" />
      <el-table-column prop="createdAt" label="导入时间" width="170" />
      <template #empty>
        <el-empty description="暂无科目成本数据" />
      </template>
    </el-table>
...
import {
  fetchCmsProductSubjectCostRows,
...
const filters = reactive({
  ...
  subjectCode: String(route.query.subjectCode || ''),
  subjectName: String(route.query.subjectName || ''),
})
const rows = ref([])
const total = ref(0)
...
async function fetchRows() {
  loading.value = true
  try {
    const page = normalizeCmsCostPage(await fetchCmsProductSubjectCostRows(buildParams()))
    rows.value = page.list
    total.value = page.total
  } catch (error) {
    ElMessage.error(error.message || 'CMS 科目成本原始数据查询失败')
  } finally {
    loading.value = false
  }
}
