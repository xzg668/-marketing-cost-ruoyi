...
describe('V3-11 联动价计算追溯页', () => {
  it('支持按 OA、客户、业务单元、价格月份、状态和变量来源筛选', () => {
    ...
    assert.match(pageContent, /label="变量来源"/)
    assert.match(pageContent, /variableSource/)
    assert.match(pageContent, /OA报价单基价覆盖/)
    assert.match(pageContent, /FINANCE_FACTOR/)
  })
  ...
  it('集中展示本单汇总和变量来源明细', () => {
    ...
    assert.match(pageContent, /OA基价覆盖变量/)
    ...
    assert.match(pageContent, /QUOTE_BASE/)
  })

  it('主表直接展示 OA 基价覆盖数量，避免必须逐行点明细', () => {
    assert.match(pageContent, /label="OA基价覆盖"/)
    assert.match(pageContent, /quoteBaseCount/)
    assert.match(pageContent, /variableSourceSummary\?\.QUOTE_BASE/)
  })
